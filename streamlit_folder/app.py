...
def Get_Reponse(user_query, chat_history, llm, retriever, ensemble_retriever):
    ...
    And in the case of the question not really requiring any content such as simple instructions such as 'remember x = 10' or simple speech such as 'thanks' or 'how are you doing?', ignore the content and respond on your own. 
    if in case the question is indeed involves the content, do use it as much as possible.
    ...
    if question_router_value.datasource == 'vectorstore':
        ...
        system = """You are a grader assessing whether an LLM generation is grounded in / supported by a set of retrieved facts and assessing whether an answer addresses / resolves a question  \n 
            ...
            You are supposed to give the binary score of 'no' if in case the answer is completely hallucination or is completely incorrect. If in case of the answer being slightly misworded or so on. Let it be."""
        ...
        while True: 
            ...
            print('answer is graded')
            # print(answer, HAOutput)
            if HAOutput.binary_score == 'yes':
                return answer
            else:
                print(f'Given Answer is incorrect. The answer: {answer}')
            # return answer
...
if __name__ == '__main__':
    ...
    st.set_page_config(page_title="Re-Search Bot: Chat")
    st.title('Re-Search Bot')
